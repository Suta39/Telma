  ...
  var sendData = {
    ...
    success: function(msg){
      if(msg=="true"){
        $.ajax({
          type: "POST",
          url: _domain+"/keiji.php?get1=image",
          //url:_domain+"/keiji/post",
          data:fd,
          processData : false,
          contentType : false,
...
function bbsDetailDom(msg,id){
  ...
  $.each(msg['comment'],function(key,val){
    ...
    listDom.find(".bbs-cmt-date").html(formatDate(new Date(val.time)));
    //var cloneDom = listDom.clone(true);
...
function locationName(id){
  ...
  t_region.name[id] = $(".location-list .edit-name")[id+1].value;
...
function locationEditAll(){
  ...
  $.post(_domain+"/keiji.php?get1=edit-all&user_id="+_user_id,sendData,
  function(res){
